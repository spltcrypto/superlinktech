...
class AvoidReuseTest(BitcoinTestFramework):
    ...
    def test_fund_send_fund_senddirty(self):
        '''
        Test the same as test_fund_send_fund_send, except send the 10 SPLT with
        the avoid_reuse flag set to false. This means the 10 SPLT send should succeed,
        ...
        # listunspent should show 1 single, unused 10 SPLT output
        assert_unspent(self.nodes[1], total_count=1, total_sum=10, reused_supported=True, reused_count=0)
        # getbalances should show no used, 10 SPLT trusted
        ...
        # listunspent should show 1 single, unused 5 SPLT output
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_supported=True, reused_count=0)
        # getbalances should show no used, 5 SPLT trusted
        ...
        # listunspent should show 2 total outputs (5, 10 SPLT), one unused (5), one reused (10)
        assert_unspent(self.nodes[1], total_count=2, total_sum=15, reused_count=1, reused_sum=10)
        # getbalances should show 10 used, 5 SPLT trusted
        assert_balances(self.nodes[1], mine={"used": 10, "trusted": 5})

        self.nodes[1].sendtoaddress(address=retaddr, amount=10, avoid_reuse=False)

        # listunspent should show 1 total outputs (5 SPLT), unused
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_count=0)
        # getbalances should show no used, 5 SPLT trusted
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 5})

        # node 1 should now have about 5 SPLT left (for both cases)
    ...
    def test_fund_send_fund_send(self, second_addr_type):
        ...
        [0] sends 10 SPLT to A.
        [1] spends 5 SPLT from A. (leaving roughly 5 SPLT useable)
        [0] sends 10 SPLT to A again.
        [1] tries to spend 10 SPLT (fails; dirty).
        [1] tries to spend 4 SPLT (succeeds; change address sufficient)
        '''
        ...
        # listunspent should show 2 total outputs (1, 10 SPLT), one unused (1), one reused (10)
        assert_unspent(self.nodes[1], total_count=2, total_sum=11, reused_count=1, reused_sum=10)
        # getbalances should show 10 used, 1 SPLT trusted
        assert_balances(self.nodes[1], mine={"used": 10, "trusted": 1})

        # node 1 should now have about 1 SPLT left (no dirty) and 11 (including dirty)
