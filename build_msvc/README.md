...
Building with Visual Studio is an alternative to the Linux based [cross-compiler build](https://github.com/spltcrypto/superlinktech/blob/master/doc/build-windows.md).
...
msbuild /m build_msvc\superlinktech.sln /p:Platform=x64 /p:Configuration=Release /t:build
```

Dependencies
---------------------
A number of [open source libraries](https://github.com/spltcrypto/superlinktech/blob/master/doc/dependencies.md) are required in order to be able to build Bitcoin Core.
...
The [external dependencies](https://github.com/spltcrypto/superlinktech/blob/master/doc/dependencies.md) required for building are:
...
To build Bitcoin Core without Qt unload or disable the `superlinktech-qt`, `libsuperlinktech_qt` and `test_superlinktech-qt` projects.
...
```
msbuild /m superlinktech.sln /p:Platform=x64 /p:Configuration=Release /p:PlatformToolset=v141 /t:build
```

- To build from the command line with the Visual Studio 2019 toolchain use:

```
msbuild /m superlinktech.sln /p:Platform=x64 /p:Configuration=Release /t:build
```

- Alternatively open the `build_msvc\superlinktech.sln` file in Visual Studio.
...
```
    #- 7z a superlinktech-%APPVEYOR_BUILD_VERSION%.zip %APPVEYOR_BUILD_FOLDER%\build_msvc\%platform%\%configuration%\*.exe
    #- path: superlinktech-%APPVEYOR_BUILD_VERSION%.zip
```
