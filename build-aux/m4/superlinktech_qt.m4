...
dnl Output: If qt version is auto, set superlinktech_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$superlinktech_qt_want_version" = xauto && test "x$superlinktech_qt_force" != xyes; then
    if test "x$superlinktech_enable_qt" != xno; then
      AC_MSG_WARN([$1; superlinktech-qt frontend will not be built])
    fi
    superlinktech_enable_qt=no
    superlinktech_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$superlinktech_enable_qt" != xno && test "x$superlinktech_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build superlinktech-qt GUI (default=auto)])],
    [
     superlinktech_qt_want_version=$withval
     if test "x$superlinktech_qt_want_version" = xyes; then
       superlinktech_qt_force=yes
       superlinktech_qt_want_version=auto
     fi
    ],
    [superlinktech_qt_want_version=auto])
...
dnl Outputs: superlinktech_enable_qt, superlinktech_enable_qt_dbus, superlinktech_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$superlinktech_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    superlinktech_enable_qt=yes
    superlinktech_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      superlinktech_enable_qt_test=no
    fi
    superlinktech_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      superlinktech_enable_qt_dbus=yes
    fi
  ...
  if test x$superlinktech_enable_qt = xyes; then
    AC_MSG_RESULT([$superlinktech_enable_qt ($QT_LIB_PREFIX)])
  else
    AC_MSG_RESULT([$superlinktech_enable_qt])
  fi
...
dnl Output: superlinktech_cv_qt5=yes|no
AC_DEFUN([_BITCOIN_QT_CHECK_QT5],[
  AC_CACHE_CHECK(for Qt 5, superlinktech_cv_qt5,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
    ...
    [superlinktech_cv_qt5=yes],
    [superlinktech_cv_qt5=no])
])])

dnl Internal. Check if the included version of Qt is greater than Qt58.
dnl Requires: INCLUDES must be populated as necessary.
dnl Output: superlinktech_cv_qt58=yes|no
AC_DEFUN([_BITCOIN_QT_CHECK_QT58],[
  AC_CACHE_CHECK(for > Qt 5.7, superlinktech_cv_qt58,[
  AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
      #include <QtCore/qconfig.h>
      #ifndef QT_VERSION
      #  include <QtCore/qglobal.h>
    ...
    [superlinktech_cv_qt58=yes],
    [superlinktech_cv_qt58=no])
])])
...
dnl Output: superlinktech_cv_static_qt=yes|no
dnl Output: Defines QT_STATICPLUGIN if plugins are static.
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, superlinktech_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
      ...
      [superlinktech_cv_static_qt=yes],
      [superlinktech_cv_static_qt=no])
    ])
  if test "x$superlinktech_cv_static_qt" = xyes; then
...
AC_DEFUN([_BITCOIN_QT_FIND_STATIC_PLUGINS],[
    if test "x$qt_plugin_path" != x; then
     ...
     m4_ifdef([PKG_CHECK_MODULES],[
       if test x$superlinktech_cv_qt58 = xno; then
       ...
       if test "x$TARGET_OS" = xwindows; then
         AC_CACHE_CHECK(for Qt >= 5.6, superlinktech_cv_need_platformsupport,[
           AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
               #include <QtCore/qconfig.h>
               #ifndef QT_VERSION
               #  include <QtCore/qglobal.h>
           ...
           [superlinktech_cv_need_platformsupport=yes],
           [superlinktech_cv_need_platformsupport=no])
         ])
         if test "x$superlinktech_cv_need_platformsupport" = xyes; then
           if test x$superlinktech_cv_qt58 = xno; then
...
dnl Inputs: superlinktech_qt_want_version (from --with-gui=). The version to check
dnl         first.
dnl Inputs: $1: If superlinktech_qt_want_version is "auto", check for this version
dnl         first.
...
dnl Inputs: superlinktech_qt_want_version (from --with-gui=). The version to use.
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITHOUT_PKGCONFIG],[
  ...
  BITCOIN_QT_CHECK([
    if test "x$superlinktech_qt_want_version" = xauto; then
