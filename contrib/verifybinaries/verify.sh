...
###   superlinktechcore.org and superlinktech.org and compares them.
...
WORKINGDIR="/tmp/superlinktech_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://superlinktechcore.org"
HOST2="https://superlinktech.org"
BASEDIR="/bin/"
VERSIONPREFIX="superlinktech-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'superlinktech-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "superlinktech.org failed to provide signature file, but superlinktechcore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "superlinktech.org and superlinktechcore.org signature files were not equal?"
