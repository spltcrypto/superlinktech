  ...
  do
    ...
    if [ "$DOWNLOAD_BINARY" -eq "0" ]; then
      ...
      if [ ! -d "$tag" ]; then
        ...
        git clone https://github.com/spltcrypto/superlinktech "$tag"
        pushd "$tag" || exit 1
        {
          git checkout "$tag"
          ...
          mkdir bin
          mv src/superlinktechd src/superlinktech-cli src/superlinktech-tx bin
          if [ "$FUNCTIONAL_TESTS" -eq "0" ]; then
            mv src/qt/superlinktech-qt bin
          fi
        }
        popd || exit 1
      fi
      ...
      if [ -d "$tag" ]; then
        ...
        if [[ "$tag" =~ v(.*)(rc[0-9]+)$ ]]; then
            BIN_PATH="bin/superlinktech-core-${BASH_REMATCH[1]}/test.${BASH_REMATCH[2]}"
        else
            BIN_PATH="bin/superlinktech-core-${tag:1}"
        fi
        URL="https://superlinktech.org/$BIN_PATH/superlinktech-${tag:1}-$PLATFORM.tar.gz"
        echo "Fetching: $URL"
        if ! curl -O -f $URL; then
            echo "Download failed."
            exit 1
        fi
        tar -zxf "superlinktech-${tag:1}-$PLATFORM.tar.gz" -C "$tag" --strip-components=1 "superlinktech-${tag:1}"
        rm "superlinktech-${tag:1}-$PLATFORM.tar.gz"
      fi
    fi
  done
}
