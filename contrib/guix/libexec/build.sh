...
# Determine output paths to use in SPLTS_* environment variables
SPLTS_GLIBC="$(store_path glibc-cross-${HOST})"
SPLTS_GLIBC_STATIC="$(store_path glibc-cross-${HOST} static)"
SPLTS_KERNEL="$(store_path linux-libre-headers-cross-${HOST})"
SPLTS_GCC="$(store_path gcc-cross-${HOST})"
SPLTS_GCC_LIBS=( "${SPLTS_GCC}/lib/gcc/${HOST}"/* ) # This expands to an array of directories...
SPLTS_GCC_LIB="${SPLTS_GCC_LIBS[0]}" # ...we just want the first one (there should only be one)

# Set environment variables to point Guix's cross-toolchain to the right
# includes/libs for $HOST
#
# NOTE: SPLTS_C_INCLUDE_PATH is missing ${SPLTS_GCC_LIB}/include-fixed, because
# the limits.h in it is missing a '#include_next <limits.h>'
#
export SPLTS_C_INCLUDE_PATH="${SPLTS_GCC_LIB}/include:${SPLTS_GLIBC}/include:${SPLTS_KERNEL}/include"
export SPLTS_CPLUS_INCLUDE_PATH="${SPLTS_GCC}/include/c++:${SPLTS_GCC}/include/c++/${HOST}:${SPLTS_GCC}/include/c++/backward:${SPLTS_C_INCLUDE_PATH}"
export SPLTS_LIBRARY_PATH="${SPLTS_GCC}/lib:${SPLTS_GCC}/${HOST}/lib:${SPLTS_GCC_LIB}:${SPLTS_GLIBC}/lib:${SPLTS_GLIBC_STATIC}/lib"

# Sanity check SPLTS_*_PATH directories
IFS=':' read -ra PATHS <<< "${SPLTS_C_INCLUDE_PATH}:${SPLTS_CPLUS_INCLUDE_PATH}:${SPLTS_LIBRARY_PATH}"
for p in "${PATHS[@]}"; do
    if [ ! -d "$p" ]; then
...
make -C depends --jobs="$MAX_JOBS" HOST="$HOST" \
                                   ...
                                   qt_config_opts_i686_linux='-platform linux-g++ -xplatform superlinktech-linux-g++'
...
if [ -z "$(find "${OUTDIR}/src" -name 'superlinktech-*.tar.gz')" ]; then
    ./autogen.sh
    env CONFIG_SITE="${BASEPREFIX}/${HOST}/share/config.site" ./configure --prefix=/
    make dist GZIP_ENV='-9n' ${V:+V=1}
    mkdir -p "${OUTDIR}/src"
    mv "$(find "${PWD}" -name 'superlinktech-*.tar.gz')" "${OUTDIR}/src/"
fi

# Determine the full path to our source tarball
SOURCEDIST="$(find "${OUTDIR}/src" -name 'superlinktech-*.tar.gz')"
# Determine our distribution name (e.g. superlinktech-0.18.0)
