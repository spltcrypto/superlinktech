...
#include <qt/superlinktechaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SuperLinkTech address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no superlinktech: URI
    if(!uri.isValid() || uri.scheme() != QString("superlinktech"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::SPLT, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("SuperLinkTech:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SPLT, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
bool openBitcoinConf()
{
    ...
    /* Open superlinktech.conf with the associated application */
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "superlinktech.desktop";
    return GetAutostartDir() / strprintf("superlinktech-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a superlinktech.desktop file to the autostart directory:
