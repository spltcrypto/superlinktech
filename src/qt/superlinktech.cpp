...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. superlinktech_de.qm (shortcut "de" needs to be defined in superlinktech.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. superlinktech_de_DE.qm (shortcut "de_DE" needs to be defined in superlinktech.qrc)
...
static const char* qt_argv = "superlinktech-qt";
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // superlinktech: URIs or payment requests:
        if (paymentServer) {
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(superlinktech);
    Q_INIT_RESOURCE(superlinktech_locale);
    ...
    /// 6. Determine availability of data directory and parse superlinktech.conf
    ...
    // superlinktech: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
