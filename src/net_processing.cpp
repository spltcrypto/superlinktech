        ...
        connman->ForNode(nodeid, [connman](CNode* pfrom){
            ...
            if (lNodesAnnouncingHeaderAndIDs.size() >= 3) {
                ...
                connman->ForNode(lNodesAnnouncingHeaderAndIDs.front(), [connman, nCMPCTBLOCKVersion](CNode* pnodeStop){
                    AssertLockHeld(cs_main);
                    connman->PushMessage(pnodeStop, CNetMsgMaker(pnodeStop->GetSendVersion()).Make(NetMsgType::SESPLTMPCT, /*fAnnounceUsingCMPCTBLOCK=*/false, nCMPCTBLOCKVersion));
                    return true;
                });
                lNodesAnnouncingHeaderAndIDs.pop_front();
            }
            connman->PushMessage(pfrom, CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::SESPLTMPCT, /*fAnnounceUsingCMPCTBLOCK=*/true, nCMPCTBLOCKVersion));
    ...
    while (!done && !orphan_work_set.empty()) {
        ...
        if (AcceptToMemoryPool(mempool, orphan_state, porphanTx, &removed_txn, false /* bypass_limits */, 0 /* nAbsurdFee */)) {
            ...
            if (!orphanTx.HasWitness() && orphan_state.GetResult() != TxValidationResult::TX_WITNESS_MUTATED) {
                ...
                // See https://github.com/spltcrypto/superlinktech/issues/8279 for details.
                assert(recentRejects);
...
bool ProcessMessage(CNode* pfrom, const std::string& msg_type, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CTxMemPool& mempool, CConnman* connman, BanMan* banman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERACK)
    {
        ...
        if (pfrom->nVersion >= SHORT_IDS_BLOCKS_VERSION) {
            ...
            if (pfrom->GetLocalServices() & NODE_WITNESS)
                connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::SESPLTMPCT, fAnnounceUsingCMPCTBLOCK, nCMPCTBLOCKVersion));
    ...
    if (msg_type == NetMsgType::SESPLTMPCT) {
        bool fAnnounceUsingCMPCTBLOCK = false;
