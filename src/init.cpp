 ...
 */
static const char* BITCOIN_PID_FILENAME = "superlinktechd.pid";
...
#endif
    gArgs.AddArg("-logtimemicros", strprintf("Add microsecond precision to debug timestamps (default: %u)", DEFAULT_LOGTIMEMISPLT), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/spltcrypto/superlinktech>";

    return (strprintf(_("Copyright (C) %i The SuperLinkTech Core Developers").translated, COPYRIGHT_YEAR) + " ") +
           "\n" +
...
void InitLogging()
{
    ...
    LogInstance().m_log_time_micros = gArgs.GetBoolArg("-logtimemicros", DEFAULT_LOGTIMEMISPLT);
#ifdef HAVE_THREAD_LOCAL
...
bool AppInitMain(NodeContext& node)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if superlinktech is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if superlinktech is started while in a temporary directory.\n",
