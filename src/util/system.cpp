...
const char * const BITCOIN_CONF_FILENAME = "superlinktech.conf";
 ...
 * https://github.com/spltcrypto/superlinktech/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //superlinktech-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "superlinktech";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SuperLinkTech
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SuperLinkTech
    // Mac: ~/Library/Application Support/SuperLinkTech
    // Unix: ~/.superlinktech
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SuperLinkTech";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SuperLinkTech";
#else
    // Unix
    return pathRet / ".superlinktech";
#endif
#endif
}
