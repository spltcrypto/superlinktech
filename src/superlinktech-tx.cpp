...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " superlinktech-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  superlinktech-tx [options] <hex-tx> [commands]  Update hex-encoded superlinktech transaction\n" +
            "or:     superlinktech-tx [options] -create [commands]   Create hex-encoded superlinktech transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded superlinktech transaction
