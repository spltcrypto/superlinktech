...
Bitcoin Core version 0.19.0 is now available from:
  ...
  <https://superlinktechcore.org/bin/superlinktech-core-0.19.0/>
=======
...
Bitcoin Core version 0.20.0 is now available from:
  ...
  <https://superlinktechcore.org/bin/superlinktech-core-0.20.0/>
...
or `superlinktechd`/`superlinktech-qt` (on Linux).
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/spltcrypto/superlinktech/issues/16599),
  [PR #16702](https://github.com/spltcrypto/superlinktech/pull/16702), and the
  `superlinktechd help` for more information.  This option is experimental and
...
- A "known bug" [announced](https://superlinktechcore.org/en/releases/0.18.0/#wallet-gui)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  program such as [HWI](https://github.com/superlinktech-core/HWI) for
...
- Bitcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.superlinktechcore.org.  (#17596)
...
- The `superlinktech-cli` utility used with the `-getinfo` parameter now
...
- The `superlinktech-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `superlinktechstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_superlinktech-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_superlinktech` binary (hebasto)
...
- #18702 Fix ASLR for superlinktech-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate superlinktech schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest superlinktech.conf (sdaftuar)
...
- #18022 Fix appveyor `test_superlinktech` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (superlinktech-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone superlinktech-core/qa-assets when fuzzing (MarcoFalke)
...
- #17119 Fix broken superlinktech-cli examples (andrewtoth)
...
- #17503 Remove bitness from superlinktech-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.superlinktechcore.org (laanwj)
...
- #17648 Rename wallet-tool references to superlinktech-wallet (hel-o)
...
- #16115 On superlinktechd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from superlinktech-cli -getinfo (malevolent)
...
- #15639 superlinktech-wallet tool: Drop libsuperlinktech_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in superlinktechd/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "superlinktech-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of superlinktechd build (NicolasDorier)
...
- #16441 remove qt libjpeg check from superlinktech_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_superlinktech to auto generated project files (sipsorcery)
...
- #15352 Reduce noise level in test_superlinktech output (practicalswift)
...
- #15896 feature_filelock, interface_superlinktech_cli: Use PACKAGE_NAME in messages rather than hardcoding Bitcoin Core (luke-jr)
...
- #16277 Suppress output in test_superlinktech for expected errors (gertjaap)
...
- #16489 log: harmonize superlinktechd logging (jonatack)
...
- #15513 add "sections" info to example superlinktech.conf (fanquake)
...
- #16080 build/doc: update superlinktech_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in superlinktech.conf (fanquake)
...
- #16621 add default superlinktech.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the superlinktech-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, superlinktech.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/superlinktechd.cpp (ch4ot1c)
...
[Transifex](https://www.transifex.com/superlinktech/superlinktech/).
