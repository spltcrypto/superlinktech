...
The headless daemon `superlinktechd` has the JSON-RPC API enabled by default, the GUI
`superlinktech-qt` has it disabled by default. This can be changed with the `-server`
...
- **Securing remote network access:** You may optionally allow other
  ...
  `superlinktechd -help` for more information about these settings and other
