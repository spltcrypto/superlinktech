Bitcoin Core version 0.18.0 is now available from:
  ...
  <https://superlinktechcore.org/bin/superlinktech-core-0.18.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://superlinktechcore.org/en/list/announcements/join/>
...
`/Applications/Bitcoin-Qt` (on Mac) or `superlinktechd`/`superlinktech-qt` (on
Linux).
...
`contrib/init/superlinktechd.service`, it has been changed to use
`/var/lib/superlinktechd` as the data directory instead of
`~superlinktech/.superlinktech`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/superlinktechd` will
exist has enough space (check using `df -h /var/lib/superlinktechd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/spltcrypto/superlinktech/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://superlinktechcore.org/en/contact/) the Bitcoin Core
...
The systemd init file (`contrib/init/superlinktechd.service`) has been changed
to use `/var/lib/superlinktechd` as the data directory instead of
`~superlinktech/.superlinktech`. This change makes Bitcoin Core more consistent with
...
`datadir` option in `/etc/superlinktech/superlinktech.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/superlinktech/superlinktech.conf`.


Documentation
-------------

- A new short [document](https://github.com/spltcrypto/superlinktech/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/spltcrypto/superlinktech/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/spltcrypto/superlinktech/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/spltcrypto/superlinktech/blob/master/doc/superlinktech-conf.md)
  about the `superlinktech.conf` file describes how to use it to configure
  Bitcoin Core.

- A new document introduces Bitcoin Core's BIP174 [Partially-Signed
  Bitcoin Transactions
  (PSBT)](https://github.com/spltcrypto/superlinktech/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/spltcrypto/superlinktech/blob/master/doc/descriptors.md)
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/spltcrypto/superlinktech/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/spltcrypto/superlinktech/blob/master/doc/descriptors.md).
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/spltcrypto/superlinktech/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/spltcrypto/superlinktech/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart superlinktechd with the `-deprecatedrpc=generate`
...
- A new `superlinktech-wallet` tool is now distributed alongside Bitcoin
...
- A sub-project of Bitcoin Core now provides Hardware Wallet Interaction
  ...
  page](https://github.com/superlinktech-core/HWI#readme) for details.
...
- #12490 Remove deprecated wallet rpc features from `superlinktech_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libsuperlinktech_server.a` (jnewbery)
...
- #13926 [Tools] superlinktech-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like superlinktechd (MarcoFalke)
...
- #14521 qt, docs: Fix `superlinktech-qt -version` output formatting (hebasto)
...
- #14204 Move `interfaces/*` to `libsuperlinktech_server` (laanwj)
...
- #15154 Configure: superlinktech-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/superlinktech-wallet (Sjors)
...
- #15110 build\_msvc: Fix the build problem in `libsuperlinktech_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in superlinktech-util-test.py (isghe)
...
- #12246 Bugfix: Only run superlinktech-tx tests when superlinktech-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_superlinktech` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_superlinktech` (MarcoFalke)
...
- #14186 superlinktech-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_superlinktech.conf` message on startup if conf file exists (leishman)
...
- #12255 Update superlinktech.service to conform to init.md (dongcarl)
...
- #14497 Add doc/superlinktech-conf.md (hebasto)
...
- #15354 Add missing `superlinktech-wallet` tool manpages (MarcoFalke)
...
- #15765 correct superlinktechconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support superlinktech folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/superlinktech/).
