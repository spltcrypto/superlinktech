Bitcoin Core version 0.17.0 is now available from:
  ...
  <https://superlinktechcore.org/bin/superlinktech-core-0.17.0/>
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `superlinktech.conf` file, not inside included files or from
  ...
  as superlinktech.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the superlinktech.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=superlinktech
    test.uacomment=superlinktech-testnet
...
be used in V0.17 by starting superlinktechd with the '-deprecatedrpc=accounts'
...
[BIP 174 PSBT](https://github.com/superlinktech-project/bips/blob/master/bip-0174.mediawiki) is an interchange format for Bitcoin transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://superlinktechtalk.org/?topic=279249) transactions.
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When superlinktech is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If superlinktech is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart superlinktechd with
...
- When running superlinktechd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If superlinktechd
...
- #11962 `0a01843` add seed.superlinktech.sprovoost.nl to DNS seeds (Sjors)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI superlinktech:// (krab)
...
- #13486 `66e1a08` Move rpc/util.cpp from libsuperlinktech-util to libsuperlinktech-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for superlinktech-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling superlinktech-qt (TheCharlatan)
...
- #12443 `07090c5` Move common args to superlinktech.conf (MarcoFalke)
...
- #12882 `d96bdd7` Make `test_superlinktech` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.superlinktech is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_superlinktech` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in superlinktechd systemd service file (Flowdalic)
- #12652 `55f490a` superlinktech-cli: Provide a better error message when superlinktechd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to superlinktechd in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for superlinktech address in translations (kaplanmaxe)
...
- #13603 `453ae5e` superlinktech-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix superlinktech-cli --version (Empact)
...
- #12877 `2b54155` Use superlinktechd in Tor documentation (knoxcard)
...
- #13346 `60f0358` update superlinktech-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust superlinktechcore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed superlinktech-cli -help output for help2man (hebasto)
