Bitcoin Core version 0.12.0 is now available from:
  ...
  <https://superlinktech.org/bin/superlinktech-core-0.12.0/>
...
[libsecp256k1](https://github.com/superlinktech-core/secp256k1) instead of OpenSSL.
...
(https://github.com/superlinktech-project/bips/blob/master/bip-0130.mediawiki)
...
(https://github.com/superlinktech-project/bips/blob/master/bip-0125.mediawiki).
...
This allows running superlinktechd without having to do any manual configuration.
...
`superlinktech.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/spltcrypto/superlinktech/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/superlinktech-project/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/superlinktech-project/bips/blob/master/bip-0065.mediawiki)
...
caching. A sample config for apache2 could look like:
    ...
    <Location /superlinktechrpc>
        ...
        # optional bypass superlinktechd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< superlinktechrpc:<password>
    </Location>
...
Restarting superlinktechd will no longer clear out the list of banned peers; instead
...
- #6269 `d4565b6` gitian: Use the new superlinktech-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include superlinktech-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix superlinktech-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of superlinktech-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [superlinktech-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libsuperlinktechconsensus (Braydon Fuller)
