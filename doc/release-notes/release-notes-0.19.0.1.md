Bitcoin Core version 0.19.0.1 is now available from:
  ...
  <https://superlinktechcore.org/bin/superlinktech-core-0.19.0.1/>
...
- [Reduce memory](https://github.com/spltcrypto/superlinktech/blob/master/doc/reduce-memory.md)
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `superlinktechd -help` output for these two options for
...
- The `testnet` field in `superlinktech-cli -getinfo` has been renamed to
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 SuperLinkTech address without
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (superlinktechVBR)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/superlinktech/superlinktech/).
